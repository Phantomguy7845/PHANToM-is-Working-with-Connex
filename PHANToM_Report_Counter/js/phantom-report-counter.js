...
(function(){
  ...
  function navUp(){
    ...
    if(last>0){ focusPath[focusPath.length-1]=last-1; }
    else if(focusPath.length>1){ focusPath.pop(); }
    const n=currentNode(); if(n){ lastFocusId=n.id; renderFocus(); }
  }
  function navDown(){
    if(!focusPath.length){ focusPath=[0]; lastFocusId=state.categories[0]?.id; renderFocus(); return; }
    let n=currentNode(); if(!n) return;
    if((n.children||[]).length){ focusPath.push(0); }
    else{
  ...
  function adjust(delta){
    const n=currentNode(); if(!n) return;
    if(n.type==="count"){ inc(n,delta); }
    else{ pulse(n.id); tip("โหมด Text: ใช้ Enter เพื่อเพิ่ม 1 บรรทัด"); }
  }
  function addLineIfText(){
    const n=currentNode(); if(!n || n.type!=="text") return;
    n.lines = n.lines||[]; n.lines.push("");
    saveState(); render();
  ...
  function miniBtn(t,fn){ const b=el("button","btn-mini",t); b.addEventListener("click",fn); return b; }
  function ghostBtn(t,fn){ const b=el("button","btn-mini btn-ghost",t); b.addEventListener("click",fn); return b; }
  function dangerBtn(t,fn){ const b=el("button","btn-mini danger",t); b.addEventListener("click",fn); return b; }
